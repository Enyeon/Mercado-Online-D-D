...
function getCurrencyConfig(systemId, code) {
    ...
    const normalizedCode = system.id === 'dnd'
        ? (DND_CURRENCY_ALIASES[code] ?? code)
        : code;
    return system.currencies.find((entry) => entry.code === normalizedCode) ?? null;
}
